...
function l = lib_names(L_path)
    ...
    d = dir( fullfile(L_path,'libopencv*') );
%     l = regexp({d.name}, 'libopencv_ts.a|libopencv_world.a|libopencv_core.so|libopencv_imgproc.so|libopencv_highgui.so', 'tokens', 'once');
    l = regexp({d.name}, 'libopencv_ts.a|libopencv_world.a|opencv_core|opencv_imgproc|opencv_highgui', 'tokens', 'once');
    l = [l{:}];
end
% mex -largeArrayDims -I'/usr/local/include' -L'/usr/local/lib'  -lopencv_core -lopencv_imgproc calcIIF.cpp MxArray.o
